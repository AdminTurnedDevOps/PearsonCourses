...
class Proto2JsonSchema {
    ...
    getBundledFileName(filename) {
        ...
        if (idx > -1) {
            const shortName = filename.substring(idx);
            if (shortName in google_types_1.googleProtoTypes) {
                return shortName;
            }
        }
        return null;
    ...
    fetch(filename, parentFilename, weak) {
        ...
        if (filename in google_types_1.googleProtoTypes) {
            this.process(filename, google_types_1.googleProtoTypes[filename]);
            return;
        }
        filename = pathUtils_1.Path.resolve(filename, parentFilename);
        if (!weak) {
    ...
    isProto3Required(field) {
        var _a;
        if ((0, protoc_gen_validate_1.isOptional)(field)) {
            return false;
        }
    ...
    compileMessage(item, stack) {
        ...
        for (const fieldName in item.fields) {
            ...
            if (field.repeated) {
                ...
                (0, protoc_gen_validate_1.visit)(properties[field.name], field);
            }
            else {
    ...
    compileField(field, parentItem, stack) {
        let obj = {};
        if (primitive_types_1.PrimitiveTypes.PRIMITIVE_TYPES_WITH_LIMITS[field.type.toLowerCase()]) {
            obj = (this.mapperOptions.primitiveTypesWithLimits) ?
                Object.assign(obj, primitive_types_1.PrimitiveTypes.PRIMITIVE_TYPES_WITH_LIMITS[field.type.toLowerCase()]) :
                Object.assign(obj, primitive_types_1.PrimitiveTypes.PRIMITIVE_TYPES_MINIMAL[field.type.toLowerCase()]);
        ...
        this.addDefaultFromCommentAnnotations(obj, field.comment);
        if (!field.repeated) {
            (0, protoc_gen_validate_1.visit)(obj, field);
        }
        const desc = this.extractDescription(field.comment);
        if (desc !== null && desc.length > 0) {
            if (obj.description) {
...
function proto2jsonSchema(rawSchema) {
    const compiler = new Proto2JsonSchema(rawSchema);
    return compiler.compile();
}
exports.proto2jsonSchema = proto2jsonSchema;
